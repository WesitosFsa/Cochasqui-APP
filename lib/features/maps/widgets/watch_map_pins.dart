...
Stream<List<MapPin>> watchMapPins(PowerSyncDatabase db, String userId) {
  return db.watch('''
    SELECT mp.id, mp.latitude, mp.longitude, mp.title, mp.description, mp.type,
           (vp.pin_id IS NOT NULL) AS visited
    FROM map_pins mp
    LEFT JOIN visited_pins vp
      ON mp.id = vp.pin_id AND vp.user_id = ?
  ''', parameters: [userId]).map((rows) {
    ...
    return rows.map((row) {
      ...
      try {
        final int id = int.parse(row['id'] as String);
